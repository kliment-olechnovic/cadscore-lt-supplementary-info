...
if [ "$MOREARGS" == "iface" ]
then
	MOREARGS="--contacts-query-inter-chain"
fi
...
/usr/bin/time -v \
${VORONOTA_DIR}/voronota-cadscore -t "$TFILE" -m "$MFILE" ${MOREARGS} --output-header \
2> "${TMPLDIR}/time_log" \
> "${TMPLDIR}/score_log"

if [ ! -s "${TMPLDIR}/time_log" ] || [ ! -s "${TMPLDIR}/score_log" ]
then
	echo "CAD-score-2 failed with: '${OUTDIR}' '${TFILE}' '${MFILE}' '${MOREARGS}'."
	exit 1
fi
