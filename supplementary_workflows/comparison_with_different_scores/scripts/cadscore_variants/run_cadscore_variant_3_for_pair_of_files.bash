...
/usr/bin/time -v \
${VORONOTA_DIR}/expansion_js/voronota-js-global-cadscore --ignore-residue-names -t "$TFILE" -m "$MFILE" ${MOREARGS} \
2> "${TMPLDIR}/time_log" \
> "${TMPLDIR}/score_log"

if [ ! -s "${TMPLDIR}/time_log" ] || [ ! -s "${TMPLDIR}/score_log" ]
then
	echo "CAD-score-3 failed with: '${OUTDIR}' '${TFILE}' '${MFILE}' '${MOREARGS}'."
	exit 1
fi
